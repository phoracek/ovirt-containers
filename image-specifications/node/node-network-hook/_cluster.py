...
class Cluster(object):
    ...
    def watch(self, resource, timeout):
        response = self._session.get(
            '{}/{}'.format(self._host, resource),
            stream=True, params={'watch': 'true', 'timeoutSeconds': timeout}
        )
        response.raise_for_status()
...
class NodeNetworkCluster(Cluster):
    ...
    def watch_network(self, timeout):
        for event in self.watch(self._resource, timeout):
